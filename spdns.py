...
def points_to_sp_sites(url):
    ...
    try:
        ...
        for rdata in answers:
            ...
            if is_sp_sites:
                print('✅ Hosted with StudioPress Sites ' +
                      '[IP: ' + ip_as_url + '].')
                return
        print('❌ A records not pointed to StudioPress Sites.')
    except dns.resolver.NoNameservers:
        print('Nameserver not reachable (SERVFAIL) for ' + url + '.')
    except dns.resolver.NXDOMAIN:
        print('DNS query names do not exist (local or test domain?).')
...
def points_to_synthesis(url):
    ...
    try:
        ...
        for rdata in answers:
            ...
            if is_synthesis:
                print('✅ Hosted with Synthesis ' + '[IP: ' + ip_as_url + '].')
                return
        print('❌ A records not pointed to Synthesis.')
    except dns.resolver.NoNameservers:
        print('Nameserver not reachable (SERVFAIL) for ' + url + '.')
    except dns.resolver.NXDOMAIN:
        print('DNS query names do not exist (local or test domain?).')
...
def points_to_rainmaker(url):
    ...
    try:
        ...
        for rdata in answers:
            ...
            if is_rainmaker:
                print('✅ Hosted with Rainmaker ' + '[IP: ' + ip_as_url + '].')
                return
        print('❌ A records not pointed to Rainmaker.')
    except dns.resolver.NoNameservers:
        print('Nameserver not reachable (SERVFAIL) for ' + url + '.')
    except dns.resolver.NXDOMAIN:
        print('DNS query names do not exist (local or test domain?).')
    ...
    try:
        ...
        for rdata in answers:
            if 'cloudflare' in rdata.to_text():
                print('✅ Using Cloudflare ' + '[NS: ' + rdata.to_text() + ']')
                return
        print('❌ Nameservers don’t appear to be Cloudflare’s.')
    except dns.resolver.NoAnswer:
        print('No NS record for ' + url + '.')
    except dns.resolver.NXDOMAIN:
        print('DNS query names do not exist (local or test domain?).')
...
def uses_wordpress_dot_com(url):
    ...
    try:
        ...
        for rdata in answers:
            ...
            if is_wp_com:
                print('✅ Hosted with WordPress.com ' + '[IP: ' + ip_as_url + '].')
                return
        print('❌ A records not pointed to WordPress.com.')
    except dns.resolver.NoNameservers:
        print('Nameserver not reachable (SERVFAIL) for ' + url + '.')
    except dns.resolver.NXDOMAIN:
        print('DNS query names do not exist (local or test domain?).')
